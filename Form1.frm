...
'''Added:  - Hi scores
'''        - Custom level
'''        - fix bug with timer after 999 second would restart
'''
'''To do:  - ? marks (maybe)
'''        - mouse up/down/move behaviour (like original Minesweeper)
'''
Option Explicit
Private Declare Function BitBlt Lib "gdi32" (ByVal hDestDC As Long, ByVal X As Long, ByVal Y As Long, ByVal nWidth As Long, ByVal nHeight As Long, ByVal hSrcDC As Long, ByVal xSrc As Long, ByVal ySrc As Long, ByVal dwRop As Long) As Long
Dim Field(29, 29) As Integer
Dim Overlay(29, 29) As Integer
...
GameLevel = 1
ScoreType = True
'Load settings and scores from file
On Error GoTo ErrorOut
Open App.Path & "\Hexsweeper.sav" For Input As #1
Line Input #1, Z
GameLevel = Z
Line Input #1, Z
LevelX = Z
...
For X = 0 To 5
    Line Input #1, Z
    formScores.Label1(X).Caption = Z
Next X
ErrorOut:
Close #1
FormCustom.Text1(0).Text = LevelX
FormCustom.Text1(1).Text = LevelY
FormCustom.Text1(2).Text = TotalMine
menuLevel_Click GameLevel   'Set level and start a new game
End Sub
...
Select Case Index
    ...
    Case 2
        ...
        TotalMine = 80
    Case 3
        LevelX = FormCustom.Text1(0).Text
        LevelY = FormCustom.Text1(1).Text
        TotalMine = FormCustom.Text1(2).Text
End Select
...
Private Sub menuBest_Click()
formScores.Show vbModal
End Sub

Private Sub Command1_Click()
New_Game
Picture2.SetFocus  'Just to get rid of that dotted box from the button
End Sub
...
Flags = TotalMine
Score
Command1.BackColor = &HC0C0C0  'vbGray
Command1.Caption = "K"  'wingding straight face
Label2.Caption = "0"
Alive = True
End Sub
...
If Timer1.Enabled = False And Label2.Caption <> "999" Then  'Start the CLOCK
    StartTime = Timer
    Timer1.Enabled = True
End If
...
Select Case Field(X, Y)
    Case 0  'Empty cell so uncover it + surounding cells
        Progress = Progress - 1
        Draw X, Y, 0
        Uncover_Cell X - 1, Y  'Yes, this Sub calls it self! (aka recursive)
        Uncover_Cell X + 1, Y
        Uncover_Cell X, Y - 1
        Uncover_Cell X, Y + 1
        If Y / 2 = Int(Y / 2) Then  'Even row
        ...
        Else                         'Odd row
            ...
            Uncover_Cell X - 1, Y + 1
        End If
    Case 1 To 6  'Numbered Cell, uncover it only
        Progress = Progress - 1
        Draw X, Y, Field(X, Y)
    Case 7  'Oh no, a mine!
        Die
...
'New high score?
If GameLevel < 3 Then
    If Val(Label2.Caption) < Val(formScores.Label1(GameLevel * 2).Caption) Then
        formScores.Label1(GameLevel * 2).Caption = Label2.Caption
        formScores.Label1(GameLevel * 2 + 1).Caption = InputBox("Enter your name for the records:", "You are the best", "Anonymous")
        If formScores.Label1(GameLevel * 2 + 1).Caption = "" Then formScores.Label1(GameLevel * 2 + 1).Caption = "Anonymous"
        formScores.Show vbModal
    End If
End If
End Sub

Private Sub Label1_Click()  'Toggle this display
ScoreType = Not ScoreType
Score
End Sub

Private Sub Score()
If ScoreType Then  'Traditional (Flags remaining to be placed)
...
Private Sub Timer1_Timer()  'Don't feed me after midnight!
Dim Temp$
Temp$ = Trim(Str(Int(Timer - StartTime)))
...
'Save settings and scores when closing
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
Dim X As Integer
On Error GoTo ErrorOut
Open App.Path & "\Hexsweeper.sav" For Output As #1
Print #1, GameLevel
Print #1, LevelX
Print #1, LevelY
Print #1, TotalMine
For X = 0 To 5
    Print #1, formScores.Label1(X).Caption
Next X
ErrorOut:
Close #1
Unload FormCustom
